...
| Phase | Work | Model |
|-------|------|-------|
| 1 Discovery | orchestrator | Sonnet 5 (main session) |
| 2 Exploration | `code-explorer-lite` | `claude-sonnet-5` |
| 3 Clarifying Questions | orchestrator | Sonnet 5 (main session) |
| 4 Architecture | `code-architect-lite` | `claude-sonnet-5` |
| 5 Implementation | orchestrator | Sonnet 5 (main session) |
| 6 Review | `code-reviewer-lite` | `claude-sonnet-5` |
| 7 Summary | `summary-writer-lite` | `claude-sonnet-5` |

Run the command on Sonnet 5 (`/model claude-sonnet-5`). Subagents carry
...
- **`code-explorer-lite`** (Sonnet 5) — graph-first exploration; fails fast if
